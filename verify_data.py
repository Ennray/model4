...
def verify_data():
    """验证导入的数据格式"""
    first_uav, second_uav = dataset()
    
    print("=== 数据验证报告 ===")
    print(f"第一批无人机(first_uav)数量: {len(first_uav)}")
    ...
    print(f"\n=== 坐标范围验证 ===")
    if first_uav:
        lats = [float(coord[0].split(':')[0]) for coord in first_uav if 'N' in coord[0]]
        lons = [float(coord[1].split(':')[0]) for coord in first_uav if 'E' in coord[1]]
        alts = [float(coord[2]) for coord in first_uav]
