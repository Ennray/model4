...
def first_uav_move_strategy(uav_speed, uav_dec_speed, uav_max_speed, enemy_speed, first_sorted_uav, first_uav_center, max_enemy_dms, acceleration, time_detect):
    first_uav_time_info = []
    start_first_uav_dms = []
    meet_first_uav_dms = []
    ...
    after_turn_single_time = [(time_detect + entry[1] + turning_time) for entry in meet_time_info]
    result = [(meet_time_info[i][0], after_turn_single_time[i], after_turn_first_uav_dms[i] ) for i in range(len(after_turn_single_time))]
    after_turn_signleuav_time = [row[1] for row in sorted(result, key=lambda x: x[0])] #对转弯后的时间按照无人机与敌群交错时间重排
    print("after_turn_singleuav_time:", after_turn_signleuav_time)
    sorted_result = sorted(result, key=lambda x: x[0])  #再按与敌群交错时间排序
    # after_single_time = sorted(after_turn_single_time, key=lambda x: x[0])
    # print("after_single_time", after_single_time)
    print("anshenmwanyip   paide :", result)
    print("result2222222222:", sorted_result)
    ...
    print("相遇时间及无人机编号：", continue_meet_time_info)
    ...
    return  firstsecond_state_dms_info, meet_first_uav_dms, after_turn_first_uav_dms, first_uav_time_info, bearing_enemy_next, after_turn_signleuav_time, continue_meet_time_info, sorted_result, turning_time
...
def generate_placements_with_bearing(
    ...
    start_side = 'left', max_rows = 999, first_uav_num = 45, second_uav_num = 30, set_second_follow = False
):
    ...
    placed_local, placements_dms = occupation_strategy(start_side, f, distance_m, first_uav_num, second_uav_num, seg_len, max_rows, inters_front, inters_back, ux, uy,
                                                       converter_enu, preplaced_local, exclusion_radius_m, alt, set_second_follow)
...
def occupation_strategy(start_side, f, distance_m, first_uav_num, second_uav_num, seg_len, max_rows, inters_front, inters_back, ux, uy,
    ...
    if set_second_follow:
        uavs_budget = first_uav_num + second_uav_num
        second_replace = True
        print("补充了几次：",uavs_budget)
    ...
    for r in range(int(max_rows)):
        ...
        for k in range(k_max + 1):
            ...
            print("增加了多少：", uavs_budget)
            if(not second_replace) and (min(lpoint_projection, rpoint_projection) < Smin) and (not set_second_follow):
...
def formation_snapshot_at_lock(pos_arrive_dms, speed, bearing_deg, time_start_lock):

    # —— 前推到锁形时刻 —— #
    pos_lock = []
    for i in range(len(pos_arrive_dms)):
        p_lock = advance_along_track_dms(pos_arrive_dms[i], bearing_deg, speed, time_start_lock)
...
def uav_turned_specific_position(enemy_center, enemy_speed, uav_center, time_info, placements_dms, first_uav_num, second_uav_num, tag_full_second):
    ...
    print("匹配关系：",tag_full_second, len(time_info), second_uav_num)
    ...
    # print("输出位置：", len(position_section))
...
def plot_positions(chase_last_uav_dms, first_uav_chase_point, second_uav_chase_point):

    # uav_first_geo_init, uav_second_geo_init,
    ...
    chase_last_lat, chase_last_lon, chase_last_alt = GeodeticConverter.decimal_dms_to_degrees(chase_last_uav_dms)  # 追击后的点位
    # #
    # # ==================================第1波无人机相遇和转弯============================================
    ...
    # }
    # ==================================追击位置============================================
    chase_first_uav_lat, chase_first_uav_lon, chase_first_alt = geo_to_degrees(first_uav_chase_point)
    chase_second_uav_lat, chase_second_uav_lon, chase_second_alt = geo_to_degrees(second_uav_chase_point)
    ...
    points = {
        ...
        'chase_last_UAV':[chase_last_lat, chase_last_lon, chase_last_alt]
    }
    ...
    scatter_points = scatter_points_0 + scatter_points_7 + scatter_points_8 #合并字典

    #存储所有点，为了找坐标端点
    merged_dict = {**points, **chase_first_uav_positions, **chase_second_uav_positions}#解压
    ...
    fig = go.Figure(scatter_points )   #+ [path]all_uav_trajectories
    # 设置显示参数
    fig.update_layout(
        scene=dict(
...
def run_port(config: dict | None = None) -> dict:
    ...
    # 增加转弯时速度为100
    uav_deceleration_speed = data.get('uav_deceleration_speed', 50)  # 默认 100
    ...
    # 先给出临时占位策略，得到无人机上的占位信息，便于追击，后续在方阵中心确定后进行重置
    front_dms, corners, placements_dms = generate_placements_with_bearing(
        ...
        preplaced_dms=[],
        start_side='left',  # 先左
        max_rows=100, first_uav_num=data['first_num'], second_uav_num=data['second_num'], set_second_follow=False
    )
    ...
    for i in range(last_uav_num):
        ...
        print("last_uavs_sorted", last_uavs_sorted)
        print("last_uavs_dms", last_uavs_dms)
        # print("当前得到的末尾无人机：", third_uav_dms)

        remain_second_dms = list(zip([id[1] for id in last_uavs_sorted], [dms[3] for dms in last_uavs_sorted]))
        remain_second_dms = remain_second_dms[1:]  #第2波次纵队中满队去掉最后一架无人机时编号+dms

        '''============================标记一下，此时的第2波次满队的无人机已经去除到了前置转弯无人机并且恢复到了原来的排序================================='''
        regression_remain_single_second = [row[1] for row in sorted(remain_second_dms, key=lambda x: x[0])]
        print("1111111111regression_remain_single_second", regression_remain_single_second)

        for dms in regression_remain_single_second:
            regression_remian_second_uavs.append(dms) #将满队去掉最后一架无人机后再进行合并(多个满队)
        ...
        print("这个才是转弯后的：",after_turn_last_dms_all)
        print("最后一架无人机时间信息：",last_time_info_all)
    ...
    # 以所有最后一架无人机的转后位置的几何中心作为我方方阵中心，重新计算占位
    if after_turn_last_dms_all:
        raw_center = calculate_center_dms(after_turn_last_dms_all)  # 我方方阵中心
        print("转完后的原始中心:", raw_center)

        # 可以在 config 里传 data['phalanx_forward_time'] 或 data['phalanx_forward_m']
        t_forward = float(data.get('phalanx_forward_time', 0.0))  # 直接给秒
        if t_forward <= 0:
            advance_m = float(data.get('phalanx_forward_m', 1500.0))  # 默认前推 1500 m
            t_forward = max(0.0, advance_m / float(data['speed']))  # 用敌速换成时间

        # 把阵心推到未来 t_forward 秒
        phalanx_center = enemy_timed_position(
            raw_center,  # 要前推的起始点（DMS）
            data['enemy_approx'],  # 敌群初始（用于求 bearing）
            data['speed'],  # 敌群速度（m/s）
            data['basepoint'],  # 基准点（bearing 的参照）
            t_forward  # 前推时长（s）
        )
        print(f"阵心前推 {t_forward:.1f}s（≈{t_forward * float(data['speed']):.0f} m）→", phalanx_center)
    ...
    def _deg_to_dms_str(val: float, is_lat: bool) -> str:
        ...
        m = int((v - d) * 60)
        s = (v - d - m / 60) * 3600
        width = 2 if is_lat else 3  # 纬度两位，经度三位
        return f"{d:0{width}d}:{m:02d}:{s:.2f}{hemi}"

    # --- 用敌群原始范围的“半宽半高”，把矩形平移到 phalanx_center ---
    lat_min_s, lat_max_s = data['enemy_latrange']
    lon_min_s, lon_max_s = data['enemy_lonrange']
    lat_min = GeodeticConverter.dms_to_decimal(lat_min_s);
    lat_max = GeodeticConverter.dms_to_decimal(lat_max_s)
    lon_min = GeodeticConverter.dms_to_decimal(lon_min_s);
    lon_max = GeodeticConverter.dms_to_decimal(lon_max_s)

    lat_half = 0.5 * (lat_max - lat_min)
    lon_half = 0.5 * (lon_max - lon_min)

    c_lat, c_lon, c_alt = GeodeticConverter.decimal_dms_to_degrees(phalanx_center)
    phalanx_latrange = [
        _deg_to_dms_str(c_lat - lat_half, True),
        _deg_to_dms_str(c_lat + lat_half, True),
    ]
    phalanx_lonrange = [
        _deg_to_dms_str(c_lon - lon_half, False),
        _deg_to_dms_str(c_lon + lon_half, False),
    ]
    print("新得到的中心范围：",phalanx_latrange,"纬度范围：",phalanx_lonrange)

    # 重算占位：阵心=我方方阵中心；朝向仍用 bearing_enemy；预放位置暂无
    front_dms, corners, placements_dms = generate_placements_with_bearing(
        phalanx_center, phalanx_latrange, phalanx_lonrange,
        distance_m=data['near_detect_distance'], bearing_deg=bearing_enemy,
        exclusion_radius_m=None, preplaced_dms=[],
        start_side='left', max_rows=100, first_uav_num=data['first_num'], second_uav_num=data['second_num'], set_second_follow = True
    )
    print("长度对不上：",len(placements_dms))

    # 给最后一架无人机们分配第2波次的槽位并追击到位
    occupied_slot_indices = set()
    for i in range(last_uav_num):
        after_turn_last_uav_dmsi = after_turn_last_dms_all[i]
        # 该机的“转完时刻”（供目标位沿敌航向前推用）
        after_turn_time = last_time_info_all[i][4]

        # 第2波次槽位索引区间：从 first_num 开始
        second_slot_start = data['first_num']
        # i==0 的“前置转弯机”指派到最近的第2波次槽位作为“中心位”
        if i == 0:
            # === 1) 先找出“最中心的 4 个槽位” ===
            # 允许配置这个数量，默认 4
            center_core_k = int(data.get('phalanx_center_k', 4))

            # 阵心（前面已经算过 phalanx_center；这里直接用）
            c_lat, c_lon, c_alt = GeodeticConverter.decimal_dms_to_degrees(phalanx_center)

            # 还未被占用的全部槽位索引
            all_free_indices = [idx for idx in range(len(placements_dms))
                                if idx not in occupied_slot_indices]

            def dist_to_center(j):
                la, lo, al = GeodeticConverter.decimal_dms_to_degrees(placements_dms[j])
                return converter.calculate_spherical_distance(c_lat, c_lon, c_alt, la, lo, al)

            # 以“到阵心的距离”排序，取最靠近阵心的前 K 个槽位
            center_candidates = sorted(all_free_indices, key=dist_to_center)[:center_core_k]

            # === 2) 在这 K 个中心槽位里，选“离当前前置机最近”的一个 ===
            lat0, lon0, alt0 = GeodeticConverter.decimal_dms_to_degrees(after_turn_last_uav_dmsi)

            def dist_to_this_uav(j):
                la, lo, al = GeodeticConverter.decimal_dms_to_degrees(placements_dms[j])
                return converter.calculate_spherical_distance(lat0, lon0, alt0, la, lo, al)

            # 若极端情况下 K=0（全部被占），就退化为“全局最近”
            candidates = center_candidates if center_candidates else all_free_indices
            center_slot_idx = min(candidates, key=dist_to_this_uav)
            ...
            # 其余“最后一架”按你的既有逻辑分配（记得跳过 occupied_slot_indices）
            idx = data['first_num'] + (i - 1)
            while idx in occupied_slot_indices and idx < len(placements_dms) - 1:
                idx += 1
            occupied_slot_indices.add(idx)
            target_slot = placements_dms[idx]
            chase_time_info, _, chase_uav_dms = last_uav_chase_strategy(
    ...
    print("前置转弯无人机总时间", last_uav_chase_time_all)
    ...
    print("排序后的1：",first_sorted)

    '''===============================目前第2处发现时间问题，已经将meet_signle_time进行修改，其为已经按编号输出的无人机转弯后的时间============================'''
    # result为转弯后的点位（已经按照编号排序）
    first_state_dms_info, meet_first_uav_dms, after_turn_first_uav_dms, first_uav_time_info, \
        bearing_enemy1, after_turn_single_time1, first_continue_meet_time, result1, turning_time1 = \
        first_uav_move_strategy(
            data['minimum_speed'], uav_deceleration_speed, data['maximum_speed'], data['speed'],
            first_sorted, first_uav_center, max_enemy_dms, data['acceleration'], time_detect
        )
    result_turn_uav_pos1 = [i[2] for i in result1]
    #当前after_turn_single_time和result1均为按照转弯时间先后排序
    print("第1波次无人机转弯后的位置：",len(result_turn_uav_pos1))
    print("first_state_dms_info", first_state_dms_info)
    print("meet_first_uav_dms", meet_first_uav_dms)
    ...
    print("剩下的第二波次无人机数:",remain_second_uav_sorted)
    second_state_dms_info, meet_second_uav_dms, after_turn_second_uav_dms, second_uav_time_info, \
        bearing_enemy2, after_turn_single_time2, second_continue_meet_time, result2, turning_time2 = \
        first_uav_move_strategy(
            data['minimum_speed'], uav_deceleration_speed, data['maximum_speed'], data['speed'],
            remain_second_uav_sorted, second_uav_center, max_enemy_dms, data['acceleration'], time_detect
        )
    result_turn_uav_pos2 = [i[2] for i in result2]
    print("第2波次无人机转弯后的位置：", len(result_turn_uav_pos2))
    ...
    print("second_slots_avail", second_slots_avail)
    ...
    print("second_assigned", second_assigned)
    ...
    dec_single_time1 = [t - last_time_info_all[0][4] for t in after_turn_single_time1]
    dec_single_time2 = [t - last_time_info_all[0][4] for t in after_turn_single_time2]

    print("这是1的什么时间：",first_uav_time_info)
    print("这是2的什么时间:", second_uav_time_info)

    print("这又是1的什么时间：",after_turn_single_time1)
    print("这又是2的什么时间：",len(dec_single_time2))
    ...
    # output_target_pos = advance_along_track_dms(phalanx_center,
    ...
    print("second_chase_info", second_chase_info)
    ...
    print("second_uav_chase_all", second_uav_chase_all)
    ...
    print("first_uav_chase_time_list",first_uav_chase_time_list)
    ...
    first_uav_chase_point = [row[1] for row in sorted(first_uav_chase_all, key=lambda x: x[0])]  #无人机编号拍的
    ...
    print("first_uav_chase_time:", first_uav_chase_time)
    ...
    for i in range(len(first_uav_chase_time)):
        first_after_chase = first_uav_chase_time[i] + after_turn_single_time1[i]
        first_after_chase_time.append(first_after_chase)

    for i in range(len(second_uav_chase_time)):
        second_after_chase = second_uav_chase_time[i] + after_turn_single_time2[i]
    ...
    time_start_lock = time_lock - last_time_info_all[0][4]

    end_square_pos = formation_snapshot_at_lock(placements_dms, data['speed'], bearing_square, time_start_lock)
    last_uav_lock_dms = [end_square_pos[i] for i in occupied_slot_indices]
    first_uav_lock_dms = [end_square_pos[i] for i in first_target_indices]
    second_uav_lock_dms =[end_square_pos[i] for i in second_target_indices]
    print("三波长度：", len(last_uav_lock_dms), len(first_uav_lock_dms), len(second_uav_lock_dms))
    print("现在的占位：", end_square_pos)

    # #时间锁时敌群中心点位置
    # enemy_center_lock = enemy_timed_position(data['enemy_approx'], data['enemy_approx'], data['speed'],
    #                                          data['basepoint'], time_lock)
    # print("敌群中心：",enemy_center_lock, "我方前置转弯", last_uav_lock_dms)
    #
    #
    #
    # chase_enemy_info = pursue_moving_point(last_uav_lock_dms[0], enemy_center_lock, bearing_enemy, data['speed'], data['speed'], data['maximum_speed'], data['acceleration'], data['acceleration'],
    #                                        30, 1, 0.1, 0.6, 20, 20000, -1)
    # chase_enmey_time = chase_time_disposal(chase_enemy_info)
    # print("两个方向：",bearing_enemy, bearing_square)
    #
    # print("chase_enemy_time", chase_enmey_time)
...
if __name__ == "__main__":
    ...
    # print("%%%%%%%",result["first_uav_chase_point"])
    # plot_positions(result["last_uav_chase_point"][0], result["first_uav_chase_point"], result["second_uav_chase_point"])
